...
// This will be a struct which has the cloud.balancer along with something else for internal load balancer. When it comes to the ensureloadbalancer
// functions, this will call the cloudprovider version for external lb, but for internal loadbalancer, it will handle it with the existing methods
var resHandler map[string]func() = make(map[string]func()){
	"backendService":
}
...
func (ilb *InternalLB) Create() {

}

func (ilb *InternalLB) checkBackendService() {

}
func (ilb *InternalLB) checkForwardingRule() {

}

func (ilb *InternalLB) checkInstanceGroup() {

}

// Instead of the above 3 functions, we can keep a common "checkResource" function and give the resource name
func (ilb *InternalLB) checkResource(resName string, resType string) {

}
func (ilb *InternalLB)  EnsureLoadBalancer(ctx context.Context, clusterName string, service *v1.Service, nodes []*v1.Node) (*v1.LoadBalancerStatus, error) {


}

func (ilb *InternalLB) GetLoadBalancerName(service *v1.Service) string {
	return ilb.namer.LoadBalancer(service.Name)
}

func (ilb *InternalLB) ensureInternalLoadBalancer(clusterName, clusterID string, svc *v1.Service, existingFwdRule *compute.ForwardingRule, nodes []*v1.Node) (*v1.LoadBalancerStatus, error) {
	...
	loadBalancerName := ilb.GetLoadBalancerName(svc)
	sharedBackend := shareBackendService(svc)
	backendServiceName := ilb.namer.ILBBackend(loadBalancerName, sharedBackend, scheme, protocol,svc.Spec.SessionAffinity)
	backendServiceLink := getBackendServiceLink(backendServiceName)
	// Ensure instance groups exist and nodes are assigned to groups
	// TODO modify the name here so it is unique for services with same name, different namespace.
	igName := makeInstanceGroupName(clusterID, svc.Name)
...
func shareBackendService(svc *v1.Service) bool {
	return cloud.GetLoadBalancerAnnotationBackendShare(svc) && !v1_service.RequestsOnlyLocalTraffic(svc)
}
...
func getBackendServiceLink(name string) string {
